...
const Navbar = () => {
  ...
  // ✅ Sync Clerk metadata role with AppContext state
  useEffect(() => {
    if (isLoaded) {
      const role = user?.publicMetadata?.role || "user";
      setIsEducator(role === "educator");
    }
  }, [isLoaded, user, setIsEducator]);
  ...
  return (
    <div className="fixed top-0 left-0 w-full z-50 flex items-center justify-between px-4 md:px-10 lg:px-24 py-2 md:py-3 border-b border-gray-300 backdrop-blur-md bg-gray-900/70 text-white shadow-md transition-all duration-300">
      ...
      <div onClick={() => navigate("/")} className="flex items-center gap-3 cursor-pointer select-none">
        <div className="w-28 h-14 md:w-32 md:h-16 rounded-xl bg-gradient-to-r from-yellow-400 to-yellow-500 flex items-center justify-center shadow-[0_4px_15px_rgba(250,204,21,0.4)] hover:shadow-[0_0_25px_rgba(250,204,21,0.7)] hover:scale-105 transition-all duration-300">
          <img
            src={assets.logo}
            alt="Aparaitech Logo"
            className="w-18 h-14 md:w-21 md:h-15 object-contain drop-shadow-[0_0_8px_rgba(0,0,0,0.4)]"
          />
        </div>
      </div>
      ...
      {/* Desktop Menu */}
      <div className="hidden md:flex items-center gap-6">
        <button onClick={() => handleNavClick("home")} className="hover:text-yellow-400 transition-colors font-medium text-sm md:text-base">
          Home
        </button>
        <button onClick={() => handleNavClick("testimonials")} className="hover:text-yellow-400 transition-colors font-medium text-sm md:text-base">
          About Us
        </button>
        <button onClick={() => handleNavClick("cta")} className="hover:text-yellow-400 transition-colors font-medium text-sm md:text-base">
          Contact Us
        </button>
        ...
        {user && (
          <>
            ...
            {/* ✅ Show Educator Dashboard ONLY if role is educator */}
            {isEducator && (
              <button
                onClick={() => navigate("/educator")}
                className="hover:text-yellow-400 transition-colors font-medium text-sm md:text-base"
              >
                Educator Dashboard
              </button>
            )}
            ...
            <span className="text-gray-400">|</span>
            <Link to="/my-enrollments" className="hover:text-yellow-400 transition-colors font-medium text-sm md:text-base">
              My Enrollments
            </Link>
          </>
        )}
        ...
        {user ? (
          ...
          <button
            onClick={() => openSignIn()}
            className="bg-gradient-to-r from-yellow-400 to-yellow-500 hover:from-yellow-500 hover:to-yellow-600 text-black font-semibold px-4 md:px-5 py-1.5 md:py-2 rounded-full shadow-md text-sm md:text-base transition-all duration-200"
          >
            Create Account
          </button>
        )}
      ...
      {mobileMenuOpen && (
        <div className="absolute top-full left-0 w-full bg-gray-900/95 text-white flex flex-col items-center py-4 md:hidden shadow-lg border-t border-gray-700">
          <button onClick={() => handleNavClick("home")} className="py-2 text-base hover:text-yellow-400">
            Home
          </button>
          <button onClick={() => handleNavClick("testimonials")} className="py-2 text-base hover:text-yellow-400">
            About Us
          </button>
          <button onClick={() => handleNavClick("cta")} className="py-2 text-base hover:text-yellow-400">
            Contact Us
          </button>
          ...
          {!user ? (
            <button
              ...
              className="mt-2 bg-gradient-to-r from-yellow-400 to-yellow-500 text-black font-semibold px-6 py-2 rounded-full shadow-md text-base transition-all duration-200"
            >
              Create Account
            </button>
          ) : (
